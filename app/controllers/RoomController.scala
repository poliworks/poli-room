  ...
  def getRooms(building: String) = Action {
    val rooms: List[Room] =
      DB readOnly { implicit session =>
        val roomDao: RoomDao = new RoomDao()
        roomDao.findAllFromBuilding(building)
      }
  ...
  def registerRoom() = Action(schemaCoerce(RegisterRoomSchema)) { request =>
    ...
    val createdRoom =
      DB localTx { implicit session =>
        val roomDao = new RoomDao()
        roomDao.registerRoom(newRoom)
      }
    Ok(Json.toJson(createdRoom))
  }

  def getRoom(id: Long) = Action {
    val room: Room = DB readOnly { implicit session =>
      new RoomDao().findById(id)
    } match {
