...
/**
  ...
  */
case class Feature(name: String, description: String, quantity: Int, img: String, id: Long = 0L) {
...
object Feature extends DatabaseModel[Feature]("features") {
  ...
  def apply(rn: scalikejdbc.ResultName[Feature])(rs: WrappedResultSet): Feature = Feature(
    rs.get(rn.name), rs.get(rn.description), rs.get(rn.quantity), rs.get(rn.img), rs.get(rn.id))

  def apply(rs: WrappedResultSet): Feature = Feature(
    rs.string("name"), rs.string("description"), rs.int("quantity"), rs.string("img"), rs.long("id"))
