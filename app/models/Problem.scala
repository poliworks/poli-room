...
/**
  ...
  */
case class Problem(name: String, description: String, reportedBy: Long, reportedAt: DateTime, featureId: Option[Long], roomId: Long, id: Long) {
...
object Problem extends DatabaseModel[Problem]("problems") {
  ...
  override def apply(rs: WrappedResultSet): Problem = Problem(
    rs.string("name"), rs.string("description"), rs.long("reported_by"), rs.jodaDateTime("reported_at"), rs.longOpt("feature_id"), rs.long("room_id"), rs.long("id"))
}
